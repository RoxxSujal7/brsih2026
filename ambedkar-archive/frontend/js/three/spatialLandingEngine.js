...
(async function () {
  ...
  function build3DExhibits() {
    ...
    // Initial exhibit is the Constitution of India
  ...
  // 10. Lighting Presets Switcher (Museum-grade Studio Illumination)
  function cycleLightingPreset() {
    const presets = ['studio_dark', 'museum_gold', 'dharma_blue', 'editorial_monochrome'];
  ...
  // 14. Scroll Handler (Apple-grade fluid Scrollytelling)
  function handleScroll() {
    ...
    // Curated Scrollytelling Choreography across the 4 archival exhibits
    if (progress < 0.28) {
      // Hero Stage: The Constitution of India (Primary national legacy)
      if (!state.userManualSelection && state.activeExhibitId !== 'constitution') {
        switchExhibit('constitution', false);
      }
      state.targetCamPos.set(0.6, 1.3, 5.0);
      state.targetLookAt.set(0.1, 0.4, 0);
    } else if (progress < 0.55) {
      // Memorial Stage: Dr. Ambedkar Memorial Bronze Bust
      if (!state.userManualSelection && state.activeExhibitId !== 'bust') {
        switchExhibit('bust', false);
      }
      state.targetCamPos.set(0.2, 1.4, 6.0);
      state.targetLookAt.set(0.1, 0.4, 0);
    } else if (progress < 0.80) {
      // Struggle & Rights Stage: Mahad Satyagraha Water Pillar
      if (!state.userManualSelection && state.activeExhibitId !== 'mahad') {
        switchExhibit('mahad', false);
      }
      state.targetCamPos.set(-2.2, 1.3, 5.2);
      state.targetLookAt.set(-0.3, 0.3, 0);
    } else {
      // Scholarly Heritage Stage: Drafting Instruments Suite
      if (!state.userManualSelection && state.activeExhibitId !== 'quill') {
        switchExhibit('quill', false);
      }
      state.targetCamPos.set(0, 2.2, 5.4);
  ...
  function animate() {
    ...
    // Rotate Cosmic Particles subtly
    if (state.particles) {
      state.particles.rotation.y = elapsed * 0.02;
      state.particles.rotation.x = elapsed * 0.01;
    }
    ...
    if (activeExhibit) {
      ...
      if (state.autoRotate && !state.isDragging) {
        state.showcaseTargetRotation.y += 0.005;
      }
      ...
      if (activeExhibit.halo) {
        activeExhibit.halo.rotation.z = elapsed * 0.4;
      }
    }
