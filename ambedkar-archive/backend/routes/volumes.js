...
// HIGH-05 FIX: Add rate limiting to volumes route
...
function loadData() {
  if (volumesCache.length === 0 && fs.existsSync(volumesPath)) {
    try { volumesCache = JSON.parse(fs.readFileSync(volumesPath, 'utf8')); } catch (e) {}
  }
  if (writingsCache.length === 0 && fs.existsSync(writingsPath)) {
    try { writingsCache = JSON.parse(fs.readFileSync(writingsPath, 'utf8')); } catch (e) {}
  }
}
...
router.get('/writings', volumesLimiter, (req, res) => {
  ...
  // HIGH-04 FIX: Validate query length
  if (q.length > 200) {
...
router.get('/:code/download', (req, res) => {
  ...
  // Fallback redirect to fast studyring archive CDN
